...
public class wte_web_api extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (canteen != null) {
			canteen_query = " AND canteen_name ILIKE '%" + canteen + "%'";
		}
		
		if (location != null) {
			location_query = " AND location ILIKE '%" + location + "%'";
		} 
		
		if (store_type != null) {
			store_type_query = " AND store_type ILIKE '%" + store_type + "%'";
		}
		...
		if (cuisine != null) {
			cuisine_query = " AND cuisine ILIKE '%" + cuisine + "%'";
		}
...
//						+ " ORDER BY canteen_name";
			...
			if(!search_string.equals(""))
				...
				query = "SELECT "+ distinct_query + " " + query_key_str +" from campus_food.campus_food WHERE (canteen_name ILIKE '%" + search_string + "%'" +
						...
						" OR room_code ILIKE '%" + search_string + "%' OR store_type ILIKE '%" + search_string + "%' " +
						" OR cuisine ILIKE '%" + search_string + "%')"  
						+ canteen_query + location_query + store_type_query + cuisine_query + halal_query + aircon_query
						+ " ORDER BY canteen_name";
			
			
		}else if(search.equals("nearby")) {
...
//		}else if(search.equals("directory")) {
//			
//			query = "SELECT "+ distinct_query + " " + query_key_str +" from campus_food.campus_food WHERE id IS NOT NULL"
//					+ location_query + store_type_query + cuisine_query
//					+ " ORDER BY canteen_name";
